...
class AuthService:
    ...
    def register_oauth_user(self, db: Session, oauth_data: schema.OAuthUserData) -> UserAccount:
    ...
    def reset_password(self, db: Session, reset_data: schema.ResetPasswordRequest) -> UserAccount:
    ...
    def change_password(
        self, db: Session, password_change: schema.ChangePasswordRequest, current_user: UserAccount
    ) -> UserAccount:
    ...
    # Account Management
    # ----------------------
    def deactivate_account(self, db: Session, user: UserAccount) -> UserAccount:
    ...
    def delete_account(self, db: Session, user: UserAccount, hard_delete: bool = False) -> bool:
    ...
    # OAuth Operations
    # ----------------------
    def link_oauth_account(self, db: Session, user: UserAccount, oauth_data: dict) -> UserAccount:
        """Link OAuth account to existing user."""
        # This would typically involve storing OAuth provider info
        pass

    def unlink_password_auth(self, db: Session, user: UserAccount) -> UserAccount:
        """Remove password authentication (convert to OAuth-only)."""
        if not user.password_hash:
            raise AuthError("User already has no password authentication")
    ...
    def _create_user_session(
        self, db: Session, user: UserAccount, refresh_token: str, request: Request
    ):
        """Create user session."""
