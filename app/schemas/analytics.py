...
class PerformanceMetrics(BaseModel):
    ...
    volatility: float = Field(
        ..., description="Volatility (standard deviation) percentage"
    )
...
class RiskMetrics(BaseModel):
    ...
    correlation_with_benchmark: float = Field(
...
class ConcentrationMetrics(BaseModel):
    ...
    top_10_weight: float = Field(
        ..., description="Weight of top 10 holdings percentage"
    )
...
class AnalyticsRequest(BaseModel):
    ...
    account_ids: Optional[List[str]] = Field(
        None, description="Specific account IDs to analyze"
    )
