...
class AnalyticsService:
    ...
    def get_portfolio_performance(
        ...
        # Get portfolio data
        portfolio_data = self._get_portfolio_data(user_id, account_ids, start_date, end_date)
        ...
        # Performance metrics
        total_return = ((portfolio_data['total_value'].iloc[-1] / portfolio_data['total_value'].iloc[0]) - 1) * 100
        annualized_return = ((portfolio_data['total_value'].iloc[-1] / portfolio_data['total_value'].iloc[0]) ** (365 / len(portfolio_data)) - 1) * 100
    ...
    def get_allocation_breakdown(
        ...
        holdings_query = self.db.query(Holding, Security, Account).join(
        ...
        for holding, security, account in holdings:
            market_value = holding.market_value or (holding.quantity * (holding.cost_basis_per_share or 0))
            data.append({
        ...
        by_asset_type = {k: round((v / total_value) * 100, 2) for k, v in by_asset_type.items()}
        ...
        if not equity_df.empty:
            ...
            by_sector = {k: round((v / total_value) * 100, 2) for k, v in by_sector.items() if k}
        else:
            by_sector = {}
        
        # Geographic allocation
        by_geography = df.groupby('country')['market_value'].sum().to_dict()
        by_geography = {k: round((v / total_value) * 100, 2) for k, v in by_geography.items() if k}
        
        # Currency allocation
        by_currency = df.groupby('currency')['market_value'].sum().to_dict()
        by_currency = {k: round((v / total_value) * 100, 2) for k, v in by_currency.items()}
        
        # Account allocation
        by_account = df.groupby(['account_id', 'account_name'])['market_value'].sum().to_dict()
        ...
        # Top holdings
        top_holdings = df.nlargest(10, 'market_value')[['symbol', 'name', 'market_value']].to_dict('records')
    ...
    def _get_portfolio_data(
        ...
        holdings_query = self.db.query(Holding, Account).join(
            Account, Holding.account_id == Account.id
        ).filter(
            Account.user_id == user_id,
            Account.is_active == True
        ...
        dates = pd.date_range(start=start_date or datetime.now() - timedelta(days=365), 
        ...
        return pd.DataFrame({'date': dates, 'total_value': total_values}).set_index('date')
    ...
    def _get_benchmark_returns(self, symbol: str) -> pd.Series:
        ...
        market_data = self.db.query(MarketData).filter(
            MarketData.security_id == benchmark.id
        ...
        df = pd.DataFrame([(md.date, md.close) for md in market_data], 
