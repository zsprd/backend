...
class CRUDMarketData(CRUDBase[MarketData, MarketDataCreate, MarketDataUpdate]):
    ...
    def get_by_security(
        ...
        *,
        ...
        if start_date:
            query = query.filter(MarketData.price_date >= start_date)
        if end_date:
            query = query.filter(MarketData.price_date <= end_date)

        query = query.order_by(desc(MarketData.price_date))
    ...
    def get_latest_prices_bulk(
        ...
        subquery = (
            ...
            .filter(MarketData.security_id.in_(security_ids))
            .group_by(MarketData.security_id)
            .subquery()
        )
        ...
        return (
            ...
            .join(
                ...
                and_(
                    ...
                    MarketData.price_date == subquery.c.max_date,
                ),
            )
            .scalars()
            .all()
        ...
        exact_match = (
            ...
            .filter(
                and_(
                    ...
                    MarketData.price_date == target_date,
                )
            )
            .first()
        )
                ...
                existing = (
                    ...
                    .filter(
                        and_(
                            ...
                            MarketData.price_date == data["price_date"],
                        )
                    )
                    .first()
                )
