...
# Test users data (this would come from your NextAuth.js token in real usage)
TEST_USER_ID = "test-users-id-123"
...
class ZSPRDAPITester:
    ...
    def test_auth_endpoints(self) -> Dict[str, Any]:
        ...
        response = requests.post(f"{self.base_url}/auth/validate-token", json=auth_data)
        print(f"Token validation status: {response.status_code}")

        # Test getting current users profile (requires valid auth)
    ...
    def test_accounts_endpoints(self) -> Dict[str, str]:
        ...
        account_data = {
            "name": "Test Investment PortfolioAccount",
            "official_name": "Test Investment PortfolioAccount - Official",
            "type": "investment",
            "subtype": "brokerage",
            "currency": "USD",
        }
        ...
        response = requests.post(
            f"{self.base_url}/accounts/", json=account_data, headers=self.headers
        )
    ...
    def test_securities_search(self) -> Dict[str, Any]:
        ...
        for query in search_queries:
            ...
            response = requests.get(
                f"{self.base_url}/market-data/search/{query}", headers=self.headers
            )
    ...
    def test_holdings_management(self, account_id: Optional[str] = None) -> Dict[str, Any]:
        ...
        holding_data = {
            ...
            "security_id": "dummy-securities-id",  # Would be real securities ID
            "quantity": 100.0,
        ...
        response = requests.post(
            f"{self.base_url}/holdings/", json=holding_data, headers=self.headers
        )
        ...
        if response.status_code == 200:
            ...
            print(
                f"Portfolio summary: ${holdings_data.get('summary', {}).get('total_market_value', 0):,.2f}"
            )
    ...
    def test_transactions_management(self, account_id: Optional[str] = None) -> Dict[str, Any]:
        ...
        transactions = [
            {
                "account_id": account_id,
                "security_id": "dummy-securities-id",
                "type": "buy",
                "side": "buy",
                "quantity": 100.0,
                "price": 150.50,
        ...
        for txn_data in transactions:
            ...
            response = requests.post(
                f"{self.base_url}/transactions/", json=txn_data, headers=self.headers
            )
    ...
    def test_analytics_endpoints(self) -> Dict[str, Any]:
        ...
        response = requests.get(f"{self.base_url}/analytics/performance", headers=self.headers)
        ...
        response = requests.get(f"{self.base_url}/analytics/risk", headers=self.headers)
        ...
        response = requests.get(f"{self.base_url}/analytics/allocation", headers=self.headers)
...
Investment PortfolioAccount,2024-01-15,AAPL,buy,buy,100,150.50,15050.00,9.95,USD,Apple stock purchase
Investment PortfolioAccount,2024-01-20,SPY,buy,buy,50,480.25,24012.50,9.95,USD,S&P 500 ETF purchase
Investment PortfolioAccount,2024-02-15,AAPL,dividend,,,,250.00,0.00,USD,Quarterly dividend
Investment PortfolioAccount,2024-03-10,AAPL,sell,sell,50,175.30,8765.00,9.95,USD,Partial Apple sale"""
...
Investment PortfolioAccount,AAPL,50,150.50,USD
Investment PortfolioAccount,SPY,50,480.25,USD
Investment PortfolioAccount,TSLA,25,220.80,USD"""
...
def main():
    ...
    # Initialize tester
    tester = ZSPRDAPITester(
        token=TEST_JWT_TOKEN if TEST_JWT_TOKEN != "your-jwt-token-here" else None
    )
